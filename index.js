...
const start = (Saraajinbot = new Client()) => {
    console.log(color(figlet.textSync('Saraajinbot', 'Larry 3D'), 'cyan'))
    ...
    console.log(color('[Saraajinbot]'), color('Saraajinbot is now online!', 'yellow'))
    ...
    Saraajinbot.onStateChanged((state) => {
        console.log(color('[Saraajinbot]'), state)
        if (state === 'UNPAIRED' || state === 'CONFLICT' || state === 'UNLAUNCHED') Saraajinbot.forceRefocus()
    })

    Saraajinbot.onAddedToGroup(async (chat) => {
        const gc = await Saraajinbot.getAllGroups()
        console.log(color('[Saraajinbot]'), 'Added a to new group. Name:', color(chat.contact.name, 'yellow'), 'Total members:', color(chat.groupMetadata.participants.length, 'yellow'))
        if (chat.groupMetadata.participants.includes(ownerNumber)) {
            await Saraajinbot.sendText(chat.id, ind.addedGroup(chat))
        } else if (gc.length > groupLimit) {
            await Saraajinbot.sendText(chat.id, `Max groups reached!\n\nCurrent status: ${gc.length}/${groupLimit}`)
            await Saraajinbot.deleteChat(chat.id)
            await Saraajinbot.leaveGroup(chat.id)
        } else if (chat.groupMetadata.participants.length < memberLimit) {
            await Saraajinbot.sendText(chat.id, `Need at least ${memberLimit} members in group!`)
            await Saraajinbot.deleteChat(chat.id)
            await Saraajinbot.leaveGroup(chat.id)
        } else {
            await Saraajinbot.sendText(chat.id, ind.addedGroup(chat))
    ...
    Saraajinbot.onMessage((message) => {
        // Uncomment code di bawah untuk mengaktifkan auto-delete cache pesan.
        /*
        Saraajinbot.getAmountOfLoadedMessages()
            .then((msg) => {
                if (msg >= 1000) {
                    console.log(color('[Saraajinbot]'), color(`Loaded message reach ${msg}, cuting message cache...`, 'yellow'))
                    Saraajinbot.cutMsgCache()
                    console.log(color('[Saraajinbot]'), color('Cache deleted!', 'yellow'))
                }
            })
        */
        
        // Comment code msgHandler di bawah untuk mengaktifkan auto-update. Kemudian, uncomment code require di bawah msgHandler.
        msgHandler(Saraajinbot, message)
        // require('./message/index.js')(Saraajinbot, message)
    })

    Saraajinbot.onIncomingCall(async (callData) => {
        await Saraajinbot.sendText(callData.peerJid, ind.blocked(ownerNumber))
        await Saraajinbot.contactBlock(callData.peerJid)
        console.log(color('[BLOCK]', 'red'), color(`${callData.peerJid} has been blocked.`, 'yellow'))
    })

    Saraajinbot.onGlobalParticipantsChanged(async (event) => {
        const _welcome = JSON.parse(fs.readFileSync('./database/group/welcome.json'))
        const isWelcome = _welcome.includes(event.chat)
        const gcChat = await Saraajinbot.getChatById(event.chat)
        const pcChat = await Saraajinbot.getContact(event.who)
        let { pushname, verifiedName, formattedName } = pcChat
        pushname = pushname || verifiedName || formattedName
        const { name, groupMetadata } = gcChat
        const botNumbers = await Saraajinbot.getHostNumber() + '@c.us'
        try {
            if (event.action === 'add' && event.who !== botNumbers && isWelcome) {
                const pic = await Saraajinbot.getProfilePicFromServer(event.who)
                if (pic === undefined) {
                ...
                await Saraajinbot.sendFile(event.chat, base64, 'welcome.png', `Welcome ${pushname}!`)
                ...
                await Saraajinbot.sendFile(event.chat, base64, 'welcome.png', `Bye ${pushname}, we will miss you~`)
            }
        } catch (err) {
            console.error(err)
        }
...
create(options(start))
    .then((Saraajinbot) => start(Saraajinbot))
